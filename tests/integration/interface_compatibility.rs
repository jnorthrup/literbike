...
use literbike::syscall_net::{
    socket_accept, socket_bind, socket_close, socket_connect, socket_create, socket_listen,
...
fn test_loopback_interface_compatibility() {
    ...
    let server_thread = thread::spawn(move || {
        ...
        let (conn_fd, peer_addr) = socket_accept(listener_fd).expect("Failed to accept connection");
    ...
    // Client side
    let client_fd =
        socket_create(libc::AF_INET, libc::SOCK_STREAM, 0).expect("Failed to create client socket");
...
fn test_physical_interface_compatibility() {
    ...
    let server_thread = thread::spawn(move || {
        let listener_fd = socket_create(libc::AF_INET, libc::SOCK_STREAM, 0)
            .expect("Failed to create listener socket");
        socket_bind(listener_fd, &addr)
            .expect("Failed to bind listener socket to physical interface");
        socket_listen(listener_fd, 1).expect("Failed to listen on socket");
    ...
    let client_fd =
        socket_create(libc::AF_INET, libc::SOCK_STREAM, 0).expect("Failed to create client socket");
    socket_connect(client_fd, &addr)
        .expect("Failed to connect client socket to physical interface");
...
fn test_virtual_interface_compatibility() {
    ...
    let server_thread = thread::spawn(move || {
        let listener_fd = socket_create(libc::AF_INET, libc::SOCK_STREAM, 0)
            .expect("Failed to create listener socket");
        socket_bind(listener_fd, &addr)
            .expect("Failed to bind listener socket to virtual interface");
        socket_listen(listener_fd, 1).expect("Failed to listen on socket");
