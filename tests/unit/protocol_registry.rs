...
use litebike::protocol_registry::{
    ProtocolRegistry, ProtocolDetector, ProtocolHandler, ProtocolDetectionResult,
    ProtocolRegistryStats, create_shared_registry
};
...
impl ProtocolDetector for TestDetector {
    ...
    fn confidence_threshold(&self) -> u8 { self.threshold }
...
impl ProtocolHandler for TestHandler {
    async fn handle(&self, _stream: PrefixedStream<TcpStream>) -> io::Result<()> {
        ...
        if self.should_fail {
            Err(io::Error::new(io::ErrorKind::Other, "Test handler failure"))
        } else {
            Ok(())
        }
    }
...
mod protocol_detection_routing_tests {
    ...
    async fn test_successful_protocol_routing() {
        ...
        let detector = Box::new(TestDetector::new("test", b"TEST".to_vec(), 200, 150));
        let handler = Box::new(TestHandler::new("test"));
        let handler_ref = handler.as_ref() as *const TestHandler;
    ...
    fn test_fallback_routing() {
        ...
        let detector = Box::new(TestDetector::new("specific", b"SPECIFIC".to_vec(), 200, 150));
