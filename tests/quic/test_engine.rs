...
async fn test_stream_creation() -> Result<()> {
    ...
    let engine = QuicEngine::new(Role::Client, initial_state, socket, addr, vec![], CoroutineContext::new());
...
async fn test_stream_id_uniqueness() -> Result<()> {
    ...
    let engine1 = QuicEngine::new(Role::Client, QuicConnectionState::default(), socket1, addr1, vec![], CoroutineContext::new());

    let socket2 = Arc::new(tokio::net::UdpSocket::bind("127.0.0.1:0").await?);
    let addr2 = "127.0.0.1:12346".parse().unwrap();
    let engine2 = QuicEngine::new(Role::Client, QuicConnectionState::default(), socket2, addr2, vec![], CoroutineContext::new());
...
async fn test_stream_multiplexing() -> Result<()> {
    let socket = Arc::new(tokio::net::UdpSocket::bind("127.0.0.1:0").await?);
    let addr = "127.0.0.1:12345".parse().unwrap();
    let engine = Arc::new(QuicEngine::new(Role::Client, QuicConnectionState::default(), socket, addr, vec![], CoroutineContext::new()));
...
async fn test_stream_reset() -> Result<()> {
    let socket = Arc::new(tokio::net::UdpSocket::bind("127.0.0.1:0").await?);
    let addr = "127.0.0.1:12345".parse().unwrap();
    let engine = QuicEngine::new(Role::Client, QuicConnectionState::default(), socket, addr, vec![], CoroutineContext::new());
...
async fn test_duplicate_ack_detection() -> Result<()> {
    let socket = Arc::new(tokio::net::UdpSocket::bind("127.0.0.1:0").await?);
    let addr = "127.0.0.1:12345".parse().unwrap();
    let engine = Arc::new(QuicEngine::new(Role::Server, QuicConnectionState::default(), socket, addr, vec![], CoroutineContext::new()));
