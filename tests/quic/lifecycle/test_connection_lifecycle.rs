...
async fn test_connection_state_transitions() {
    ...
    let engine = QuicEngine::new(
        Role::Client,
        QuicConnectionState::new(),
        Arc::new(socket),
        remote_addr,
    );
    
    // Test initial state
    assert_eq!(engine.get_connection_state(), ConnectionState::Initial);
    
    // Test state transitions
    engine.set_connection_state(ConnectionState::Handshaking);
    assert_eq!(engine.get_connection_state(), ConnectionState::Handshaking);
    
    engine.set_connection_state(ConnectionState::Established);
    assert_eq!(engine.get_connection_state(), ConnectionState::Established);
    
    engine.set_connection_state(ConnectionState::Closing);
    assert_eq!(engine.get_connection_state(), ConnectionState::Closing);
    
    engine.set_connection_state(ConnectionState::Closed);
    assert_eq!(engine.get_connection_state(), ConnectionState::Closed);
}
