...
impl BenchmarkRunner {
        ...
        T: Clone,
    {
        ...
        let memory_before = if self.memory_tracking {
            get_memory_usage()
        } else {
            0
        };
        ...
        let metrics =
            PerformanceMetrics::from_durations(&durations).with_memory_usage(memory_usage);
        ...
        D: literbike::protocol_registry::ProtocolDetector,
    {
        ...
        println!(
            "{:<15} {:<12} {:<12} {:<12} {:<15}",
            "Detector", "Avg (μs)", "95th (μs)", "99th (μs)", "Ops/sec"
        );
        println!("{:-<70}", "");

        for (name, metrics) in &results {
            println!(
                "{:<15} {:<12.2} {:<12.2} {:<12.2} {:<15.2}",
                name,
...
impl ConcurrentTester {
    ...
    where
        D: literbike::universal_listener::ProtocolDetector
            + Send
            + Sync
            + 'static
...
impl ThroughputResult {
    pub fn print_summary(&self) {
        ...
        println!(
            "  Connections: {} ({:.2}/sec)",
            self.connection_count, self.connections_per_second
        );
        println!("  Errors: {}", self.error_count);
        println!(
            "  Bytes sent: {} ({:.2} bytes/sec)",
            self.total_bytes_sent, self.bytes_per_second_sent
        );
        println!(
            "  Bytes received: {} ({:.2} bytes/sec)",
            self.total_bytes_received, self.bytes_per_second_received
        );
        println!(
            "  Bandwidth utilization: {:.2} MB/s up, {:.2} MB/s down",
