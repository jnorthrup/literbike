        ...
        #[cfg(target_os = "linux")]
        {
            match UringNative::new(entries) {
                ...
                Err(e) => {
                    ...
                    Ok(Self::Emulator(Arc::new(Mutex::new(UringEmulator::new(
                        entries as usize,
                    )?))))
                }
            }
    ...
    pub fn submit(self) -> io::Result<()> {
        match &self.op {
            OpBuilder::Read { len } => {
                ...
                self.uring
                    .backend
                    .queue_read(self.fd, &mut buf, self.user_data)?;
            }
            OpBuilder::Write { len } => {
                let buf = vec![0u8; *len];
                self.uring
                    .backend
                    .queue_write(self.fd, &buf, self.user_data)?;
            }
            OpBuilder::ReadAt { offset, len } => {
                let mut buf = vec![0u8; *len];
                self.uring
                    .backend
                    .queue_read_at(self.fd, *offset, &mut buf, self.user_data)?;
            }
            OpBuilder::WriteAt { offset, len } => {
                let buf = vec![0u8; *len];
                self.uring
                    .backend
                    .queue_write_at(self.fd, *offset, &buf, self.user_data)?;
            }
            OpBuilder::Nop => {
            ...
            OpBuilder::PollAdd(mask) => {
                self.uring
                    .backend
                    .queue_poll_add(self.fd, *mask, self.user_data)?;
            }
        }
        Ok(())
    }
