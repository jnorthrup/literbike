pub fn render_toolbar_ui() -> String {
    r#"<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
#[cfg(test)]
mod tests {
    use super::render_toolbar_ui;

    #[test]
    fn toolbar_ui_embeds_titlebar_icon_and_menu_shell() {
        let html = render_toolbar_ui();
        assert!(html.contains("rel=\"icon\""));
        assert!(html.contains("<svg class=\"menu-icon\""));
        assert!(html.contains("titlebar-style menu"));
    }

    #[test]
    fn toolbar_ui_targets_live_toolbar_endpoints() {
        let html = render_toolbar_ui();
        assert!(html.contains("fetch('/toolbar/state')"));
        assert!(html.contains("fetch('/toolbar/actions'"));
        assert!(html.contains("set_streaming_enabled"));
    }
}
