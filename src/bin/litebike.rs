...
fn run_route_cmd(args: &[String]) {
    run_route(args);
}
fn run_probe_cmd(args: &[String]) {
    run_probe(args);
}
fn run_domains_cmd(args: &[String]) {
    run_domains(args);
}
fn run_carrier_cmd(args: &[String]) {
    run_carrier(args);
}
fn run_scan_ports_cmd(args: &[String]) {
    run_scan_ports(args);
}
fn run_bonjour_discover_cmd(args: &[String]) {
    run_bonjour_discover(args);
}
...
mod tests {
    ...
    fn test_run_ifconfig_with_filter_does_not_panic() {
        ...
        let first = list_interfaces()
            .ok()
            .and_then(|m| m.keys().next().cloned());
        if let Some(ifn) = first {
            run_ifconfig(&[ifn]);
        } else {
            run_ifconfig(&[]);
        }
    ...
    #[test]
    fn test_run_ifconfig_mac_branch_executes() {
        run_ifconfig(&[]);
    }
...
fn run_ip(args: &[String]) {
    ...
    match args[subcmd_idx].as_str() {
        "addr" | "address" => match list_interfaces() {
            Ok(ifaces) => {
                let mut idx = 1u32;
                for (name, iface) in ifaces {
                ...
                match get_default_gateway_v6() {
                    ...
                    Err(e) => {
                        ...
                        if let Ok(ip) = get_default_local_ipv6() {
                            let iface =
                                guess_default_v6_interface().unwrap_or_else(|| "-".to_string());
    ...
    match gw {
        ...
        Err(e) => {
            ...
            if let Ok(ip) = get_default_local_ipv4() {
                println!(
                    "(hint) default local IPv4: {} (gateway likely {}.1)",
                    ip,
                    ip.to_string()
                        .rsplit('.')
                        .skip(1)
...
fn run_netstat(args: &[String]) {
    ...
    if show_routes {
        return run_netstat_route();
    }
    if show_ifaces {
        return run_netstat_interfaces();
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "android"))]
    {
        ...
        if File::open("/proc/net/tcp").is_ok() {
            ...
            if print_proc_net_sockets_filtered(show_tcp, show_udp, listening_only) {
                return;
            }
        }
        ...
        if print_external_netstat(&["netstat", "-an"]) {
            return;
        }
        if print_external_netstat(&["busybox", "netstat", "-an"]) {
            return;
        }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if print_external_netstat(&["netstat", args_vec.join(" ").as_str()]) {
            return;
        }
...
fn print_external_netstat(cmd: &[&str]) -> bool {
    use literbike::syscall_net::{list_interfaces, Interface, InterfaceAddr};
    use std::process::Command;
    if cmd.is_empty() {
        return false;
    }
    ...
    match get_default_gateway() {
        ...
        Err(e) => {
            ...
            if let Ok(ip) = get_default_local_ipv4() {
                ...
                if octets.len() == 4 {
                    println!(
                        "(hint) default local IPv4 {} (gw likely {}.1)",
                        ip,
                        [octets[0], octets[1], octets[2]].join(".")
                    );
                }
            }
        }
        ...
        Ok(ifaces) => {
            for (name, iface) in ifaces {
                ...
                let v6count = iface
                    .addrs
                    .iter()
                    .filter(|a| matches!(a, InterfaceAddr::V6(_)))
                    .count();
                let mac = iface
                    .addrs
                    .iter()
                    .find_map(|a| {
                        if let InterfaceAddr::Link(m) = a {
                            Some(m.clone())
                        } else {
                            None
                        }
...
fn run_watch(args: &[String]) {
    ...
    fn snapshot(include_v6: bool) -> Snap {
        ...
        if let Ok(ifaces) = list_interfaces() {
            for (name, iface) in ifaces {
                ...
                for a in iface.addrs {
                    match a {
                        InterfaceAddr::V4(ip) => {
                            v4.insert(ip.to_string());
                        }
                        InterfaceAddr::V6(ip) => {
                            if include_v6 {
                                v6.insert(ip.to_string());
                            }
                        }
                        InterfaceAddr::Link(_) => {}
                    }
        ...
        if let Ok(gw) = get_default_gateway() {
            snap.gw4 = Some(gw.to_string());
        }
        if include_v6 {
            snap.v6_iface = guess_default_v6_interface();
        }
        snap
    }
    ...
    fn print_diff(prev: &Snap, curr: &Snap, t: &str) {
        ...
        for (name, ci) in &curr.ifs {
            if let Some(pi) = prev.ifs.get(name) {
                for ip in ci.v4.difference(&pi.v4) {
                    println!("{} {} v4+ {}", t, name, ip);
                }
                for ip in pi.v4.difference(&ci.v4) {
                    println!("{} {} v4- {}", t, name, ip);
                }
                for ip in ci.v6.difference(&pi.v6) {
                    println!("{} {} v6+ {}", t, name, ip);
                }
                for ip in pi.v6.difference(&ci.v6) {
                    println!("{} {} v6- {}", t, name, ip);
                }
                if ci.flags != pi.flags {
                    println!("{} {} flags 0x{:x}->0x{:x}", t, name, pi.flags, ci.flags);
                }
            }
        }
        // Default v4 GW changes
        if prev.gw4 != curr.gw4 {
            println!(
                "{} default-v4 {}->{}",
                t,
                prev.gw4.clone().unwrap_or("-".into()),
                curr.gw4.clone().unwrap_or("-".into())
            );
        }
        // v6 egress iface hint
        if prev.v6_iface != curr.v6_iface {
            println!(
                "{} default-v6-iface {}->{}",
                t,
                prev.v6_iface.clone().unwrap_or("-".into()),
                curr.v6_iface.clone().unwrap_or("-".into())
            );
        }
    }
...
fn run_probe(_args: &[String]) {
    ...
    match get_default_local_ipv6() {
        Ok(ip6) => {
            ...
            let gw6 = get_default_gateway_v6()
                .map(|g| g.to_string())
                .unwrap_or_else(|_| "-".into());
            println!(
                "v6: src {} iface {} gw {} ({})",
        ...
        Ok(ifaces) => {
            for (name, iface) in ifaces {
                ...
                for a in iface.addrs {
                    match a {
                        InterfaceAddr::V4(ip) => {
                            v4_classes.push(classify_ipv4(ip));
                            if first_v4.is_none() {
                                first_v4 = Some(ip);
                            }
                        }
                ...
                let domain = if name.starts_with("rmnet")
                    || name.starts_with("ccmni")
                    || name.starts_with("wwan")
                {
                    "cell"
                } else if name.starts_with("wlan") || name.starts_with("swlan") {
                    "wifi"
                ...
                let v6hint = v6_classes
                    .iter()
                    .find(|&&c| c == "global")
                    .copied()
                    .unwrap_or(v6_classes.get(0).copied().unwrap_or("-"));
                println!(
            ...
            if let Ok(gw) = get_default_gateway() {
                println!("default v4 gw {}", gw);
            }
            if let Ok(gw6) = get_default_gateway_v6() {
                println!("default v6 gw {}", gw6);
            } else if let Some(ifn) = guess_default_v6_interface() {
                println!("(hint) default v6 egress iface {}", ifn);
            }
        }
...
fn run_carrier(_args: &[String]) {
    #[cfg(any(target_os = "android"))]
    {
        ...
        if props.is_empty() {
            println!("carrier: no getprop keys visible (managed device?)");
            return;
        }
...
fn run_radios(args: &[String]) {
    ...
    if ssh_target.is_none() {
        if let Ok(env_host) = std::env::var("LITEBIKE_SSH") {
            if !env_host.trim().is_empty() {
                ssh_target = Some(env_host);
            }
        }
    }
        ...
        if h.is_empty() {
            ...
            if let Ok(home) = std::env::var("HOME") {
                ...
                if let Ok(s) = std::fs::read_to_string(&p) {
                    let t = s.trim();
                    if !t.is_empty() {
                        inferred = Some(t.to_string());
                    }
                }
                if inferred.is_none() {
                    let p2 = std::path::Path::new(&home).join(".litebike-ssh");
                    if let Ok(s) = std::fs::read_to_string(&p2) {
                        let t = s.trim();
                        if !t.is_empty() {
                            inferred = Some(t.to_string());
                        }
            ...
            if inferred.is_none() {
                if let Ok(s) = std::fs::read_to_string("ssh_target") {
                    let t = s.trim();
                    if !t.is_empty() {
                        inferred = Some(t.to_string());
                    }
    ...
    if let Some(host) = ssh_target {
        ...
        let mut has_shkc = false;
        for t in &opts_vec {
            let lt = t.to_lowercase();
            if lt.contains("batchmode") {
                has_batch = true;
            }
            if lt.contains("connecttimeout") {
                has_cto = true;
            }
            if lt.contains("stricthostkeychecking") {
                has_shkc = true;
            }
        }
        if !has_batch {
            opts_vec.push("-o".into());
            opts_vec.push("BatchMode=yes".into());
        }
        if !has_cto {
            opts_vec.push("-o".into());
            opts_vec.push("ConnectTimeout=5".into());
        }
        if !has_shkc {
            opts_vec.push("-o".into());
            opts_vec.push("StrictHostKeyChecking=accept-new".into());
        }
        ...
        match out1 {
            ...
            _ => {
                ...
                let out3 = cmd3
                    .arg(&host)
                    .arg("ifconfig -a || ifconfig || ip addr")
                    .output();
                match out3 {
...
fn run_snapshot(args: &[String]) {
    ...
    if let Err(e) = fs::create_dir_all(&dir) {
        eprintln!("snapshot: cannot create dir {}: {}", dir.display(), e);
        return;
    }
    ...
    match list_interfaces() {
        Ok(ifaces) => {
            for (name, iface) in ifaces {
                ...
                out.push_str(&format!(
                    "{} flags=0x{:x} idx={} v4=[{}] v6=[{}] mac={}\n",
                    name,
                    iface.flags,
                    iface.index,
                    v4s.join(","),
                    v6s.join(","),
                    if mac.is_empty() { "-".into() } else { mac }
                ));
            }
        }
    ...
    match get_default_gateway() {
        Ok(gw) => out.push_str(&format!("v4_gw={}\n", gw)),
        Err(e) => out.push_str(&format!("v4_gw=error({})\n", e)),
    }
    match get_default_gateway_v6() {
        Ok(gw6) => out.push_str(&format!("v6_gw={}\n", gw6)),
        Err(_) => {
            if let Some(ifn) = guess_default_v6_interface() {
                out.push_str(&format!("v6_iface_hint={}\n", ifn));
            } else {
                out.push_str("v6=unavailable\n");
            }
        }
    }
    out.push('\n');
            ...
            for (name, iface) in ifaces {
                ...
                for a in iface.addrs {
                    match a {
                        InterfaceAddr::V4(ip) => {
                            if !has_v4 {
                                v4hint = classify_ipv4(ip);
                            }
                            has_v4 = true;
                        }
                        InterfaceAddr::V6(ip) => {
                            if !has_v6 {
                                v6hint = classify_ipv6(ip);
                            }
                            has_v6 = true;
                        }
                        InterfaceAddr::Link(_) => {}
    ...
    if let Err(e) = fs::write(&path, out) {
        eprintln!("snapshot: failed to write {}: {}", path.display(), e);
    } else {
...
fn run_proxy_setup(args: &[String]) {
        ...
        match cmd {
            "enable" => {
                ...
                .is_ok()
                {
                    ...
                    println!(
                        "⚠ Proxy port {} not yet reachable (service may need to be started)",
                        proxy_port
                    );
                }
    ...
    {
        eprintln!(
            "proxy-setup: only available on macOS (command '{}' ignored)",
            cmd
        );
    }
}
...
fn run_upnp_gateway(args: &[String]) {
    ...
    let local_ip =
        get_default_local_ipv4().unwrap_or_else(|_| std::net::Ipv4Addr::new(127, 0, 0, 1));
            ...
            loop {
                if let Ok((len, src)) = socket.recv_from(&mut buf) {
                    ...
                    if msg.contains("M-SEARCH")
                        && (msg.contains("ssdp:all") || msg.contains("InternetGatewayDevice"))
                    {
                        let response = format!(
                            "HTTP/1.1 200 OK\r\n\
                        ...
                        Classify::Protocol(protocol) => {
                            match protocol {
                                ...
                                ProtocolType::Http(_method) => {
                                    ...
                                    if request_str.contains("GET /proxy.pac")
                                        || request_str.contains("GET /wpad.dat")
                                    {
                                        // Serve PAC file
                                        let response = format!(
                                            "HTTP/1.1 200 OK\r\n\
                                                        ...
                                                        ))
                                                    {
                                                            ...
                                                            .is_ok()
                                                        {
                                                            ...
                                                            let mut response_buffer =
                                                                vec![0u8; 8192];
                                                            if let Ok(bytes_read) = target_stream
...
fn run_proxy_cleanup(args: &[String]) {
    ...
    if turds_found.is_empty() {
        ...
        println!(
            "\n✅ Cleaned {} proxy turds from the system",
            turds_cleaned.len()
        );
    }
...
fn run_ssh_deploy(args: &[String]) {
    let mut termux_host =
        std::env::var("TERMUX_HOST").unwrap_or_else(|_| "192.168.1.100".to_string());
...
"#;
    ...
    println!("💡 Next steps:");
    println!(
        "   1. Test: curl -x http://{}:8080 http://httpbin.org/ip",
        termux_host
    );
    println!(
        "   2. Configure: litebike proxy-config --host {}",
        termux_host
    );
    println!(
        "   3. Monitor: ssh -p {} {}@{} 'tail -f knox-proxy.log'",
        termux_port, termux_user, termux_host
    );
}
...
fn run_bootstrap(args: &[String]) {
    ...
    let self_modified = fs::metadata(&self_exe)
        .ok()
    ...
    if has_cargo_cache && has_cargo_lock {
        ...
        let rsync_result = Command::new("rsync")
            .args([
                "-avz",
                &format!("{}:~/.cargo/registry/", peer),
                &format!("{}/.cargo/", cargo_home),
            ])
            .status();
    ...
    if new_exe.exists() {
        let new_modified = fs::metadata(new_exe)
            .ok()
            .and_then(|m| m.modified().ok())
            .unwrap_or_else(|| std::time::SystemTime::UNIX_EPOCH);
        ...
        if new_modified > self_modified {
            ...
            println!(
                "   Manual: cp target/release/litebike {}",
                self_exe.display()
            );
        } else {
...
fn run_pattern_match(args: &[String]) {
    ...
    match pattern_type.as_str() {
        ...
        "regex" => match rbcursive.match_regex(&data, pattern) {
            Ok(result) => {
                if result.matched {
                    println!(
                        "✅ Regex pattern '{}' matched ({} matches)",
                        pattern, result.total_matches
                    );
                    for (i, m) in result.matches.iter().enumerate() {
                        println!("  Match {}: bytes {}..{}", i + 1, m.start, m.end);
        ...
        _ => {
            eprintln!(
                "Invalid pattern type '{}'. Use 'glob' or 'regex'",
                pattern_type
            );
        }
    }
...
fn run_pattern_scan(args: &[String]) {
    if args.len() < 2 {
        ...
        println!("");
        println!(
            "This command uses SIMD-accelerated scanning for better performance on large data."
        );
        return;
    }
...
fn run_pattern_bench(args: &[String]) {
    ...
    for pattern in &regex_patterns {
        ...
        for _ in 0..iterations {
            let _ = rbcursive.scan_with_pattern(
                &test_data,
                pattern,
                literbike::rbcursive::PatternType::Regex,
            );
        }
        ...
        println!(
            "   Pattern '{}': {:.2} MB/s (avg: {:?})",
            pattern, throughput_mb_s, avg_time
        );
    }

    // Benchmark glob pattern matching
    println!("\n🌐 Glob Pattern Benchmarks:");
    let glob_patterns = ["*.txt", "test*", "*api*", "GET*HTTP*"];
    ...
    for pattern in &glob_patterns {
        ...
        for _ in 0..iterations {
            let _ = rbcursive.scan_with_pattern(
                &test_data,
                pattern,
                literbike::rbcursive::PatternType::Glob,
            );
        }
