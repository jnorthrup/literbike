...
pub trait PatternMatcher: Send + Sync {
    ...
    fn find_all_regex(&self, data: &[u8], pattern: &str)
...
impl SimdPatternMatcher {
    ...
    fn get_glob(&mut self, pattern: &str) -> Result<&GlobPattern, PatternError> {
        if !self.glob_cache.contains_key(pattern) {
            ...
            let glob =
                GlobPattern::new(pattern).map_err(|e| PatternError::InvalidGlob(e.to_string()))?;
...
impl PatternMatcher for SimdPatternMatcher {
    fn match_glob(&self, data: &[u8], pattern: &str) -> PatternMatchResult {
        ...
        match Self::bytes_to_str(data) {
            Ok(text) => match matcher.get_glob(pattern) {
                Ok(glob) => {
...
impl PatternScanner {
    ...
    pub fn simd_guided_pattern_scan(
        &self,
        data: &[u8],
        pattern: &str,
        pattern_type: PatternType,
...
mod tests {
    ...
    fn test_regex_pattern_matching() {
        ...
        let result = matcher
            .match_regex(data, r"GET /api/v\d+/users/(\d+)")
            .unwrap();
        assert!(result.matched);
        assert_eq!(result.total_matches, 1);
    ...
    fn test_pattern_scanner() {
        ...
        let matches = scanner
            .scan_with_pattern(file_data, "*.json", PatternType::Glob)
            .unwrap();
    ...
    fn test_simd_guided_pattern_scan() {
        ...
        let matches = scanner
            .simd_guided_pattern_scan(data, r"(GET|POST|PUT)", PatternType::Regex)
            .unwrap();
