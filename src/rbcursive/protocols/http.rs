...
use super::{HttpHeader, HttpRequest, HttpVersion};
use crate::rbcursive::{
    combinators::*, scanner::SimdScanner, simd::create_optimal_scanner, HttpMethod,
};
...
impl HttpParser {
    ...
    fn parse_request_line<'a>(
        &self,
        input: &'a [u8],
    ) -> ParseResult<(HttpMethod, &'a [u8], HttpVersion), ParseError> {
        // Parse method
        ...
        let (_, space2_consumed) = match space_parser.parse(remaining) {
            ParseResult::Complete(s, c) => (s, c),
            ParseResult::Incomplete(c) => {
                return ParseResult::Incomplete(
                    method_consumed + space_consumed + path_consumed + c,
                )
            }
            ParseResult::Error(e, c) => {
                return ParseResult::Error(e, method_consumed + space_consumed + path_consumed + c)
            }
        };
        ...
        let (version, version_consumed) = match version_parser.parse(remaining) {
            ...
            ParseResult::Incomplete(c) => {
                return ParseResult::Incomplete(
                    method_consumed + space_consumed + path_consumed + space2_consumed + c,
                )
            }
            ParseResult::Error(e, c) => {
        ...
        let total_consumed =
            method_consumed + space_consumed + path_consumed + space2_consumed + version_consumed;
    ...
    fn parse_headers<'a>(&self, input: &'a [u8]) -> ParseResult<Vec<HttpHeader<'a>>, ParseError> {
        ...
        loop {
            ...
            let header_consumed = colon_pos
                + 1
                + (after_colon.as_ptr() as usize - remaining.as_ptr() as usize - colon_pos - 1)
                + line_end_consumed;
