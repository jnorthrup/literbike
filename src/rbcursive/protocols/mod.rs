    ...
    match p.parse_method(data).signal() {
        Signal::Accept => {
            ...
            match p.parse_method(data) {
                crate::rbcursive::combinators::ParseResult::Complete(m, _) => {
                    Classify::Protocol(ProtocolType::Http(m))
                }
...
fn socks5_classify(data: &[u8]) -> Classify {
    ...
    if data[0] == 0x05 {
        Classify::Protocol(ProtocolType::Socks5)
    } else {
        Classify::Unknown
    }
}
...
fn json_classify(data: &[u8]) -> Classify {
    ...
    for i in 1..limit {
        if data[i] == b'}' {
            return Classify::Protocol(ProtocolType::Json);
        }
    }
    Classify::NeedMore
}
...
fn tls_classify(data: &[u8]) -> Classify {
    ...
    if data[0] == 0x16 && data[1] == 0x03 {
        Classify::Protocol(ProtocolType::Tls)
    } else {
        Classify::Unknown
    }
}
...
pub static PROTOCOL_SPECS: &[ProtocolSpec] = &[
    ...
    ProtocolSpec {
        name: "socks5",
        anchors: &[Anchor::Literal(&[0x05])],
        classify: socks5_classify,
    },
    ProtocolSpec {
        name: "json",
        anchors: &[Anchor::Confix {
            open: b'{',
            close: b'}',
        }],
        classify: json_classify,
    },
    ProtocolSpec {
        name: "tls",
        anchors: &[Anchor::Literal(&[0x16, 0x03])],
        classify: tls_classify,
    },
    ProtocolSpec {
        name: "dns",
        anchors: &[Anchor::Range {
            start: 0x00,
            end: 0xFF,
            min: 2,
        }],
        classify: dns_classify,
    },
];
...
impl<const N: usize> Listener<N> {
    pub const fn new(specs: &'static [ProtocolSpec; N]) -> Self {
        Self { specs }
    }
