...
use crate::rbcursive::combinators::{ParseError, ParseResult, Parser};
...
impl MultiStreamParser {
    ...
    pub fn feed_and_try<T>(
        &mut self,
        data: &[u8],
        parsers: &[&dyn for<'a> Parser<'a, T, Error = ParseError>],
    ) -> MultiParseResult<T>
    where
        T: Clone,
    {
...
mod tests {
    use super::*;
    use crate::rbcursive::combinators::{byte, sequence, tag};
    ...
    fn test_stream_parser_basic() {
        ...
        assert!(matches!(
            result,
            StreamFeedResult::Ok | StreamFeedResult::DataAdded(_)
        ));
    ...
    fn test_stream_parser_http_request() {
        ...
        parser.feed(b"GET");
        assert!(matches!(
            parser.try_parse(GetPathUnit),
            StreamParseResult::NeedMoreData(_)
        ));
