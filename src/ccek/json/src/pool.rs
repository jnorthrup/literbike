...
///
/// ```rust
/// use ccek_json::pool::AtomicPool;
///
/// let pool = AtomicPool::new();
///
/// // Get an object from the pool (or create new)
/// let mut obj = pool.get_or_create(|| Vec::new());
///
/// // Use the object
/// obj.push(1);
///
...
///
/// ```rust
/// use ccek_json::pool::{AtomicPool, Pooled};
///
/// let pool: AtomicPool<Vec<u8>> = AtomicPool::new();
...
mod tests {
    ...
    fn test_pool_concurrent() {
        let pool: AtomicPool<Vec<usize>> = AtomicPool::new();
        let mut handles = vec![];
    ...
    fn test_pool_clear() {
        ...
        // Collect objects first so they aren't reused, then put them all back
        let mut vecs: Vec<Vec<u8>> = (0..10)
            .map(|_| pool.get_or_create(|| Vec::new()))
            .collect();
