...
impl DataChunk {
    ...
    pub fn from_bytes(mut bytes: &[u8]) -> io::Result<Self> {
        ...
        // Use the header length to determine actual data extent (excludes padding)
        let value_len = header.value_length();
        bytes = &bytes[ChunkHeader::SIZE..];

        if bytes.len() < 12 || value_len < 12 {
            return Err(io::Error::new(
                io::ErrorKind::UnexpectedEof,
        ...
        let transmission_seq_num = u32::from_be_bytes([bytes[8], bytes[9], bytes[10], bytes[11]]);
        // Use header length to exclude padding bytes
        let data_end = value_len.min(bytes.len());
        let user_data = bytes[12..data_end].to_vec();
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let value_len = 12 + self.user_data.len();
...
impl InitChunk {
    pub fn from_bytes(bytes: &[u8]) -> io::Result<Self> {
        let _header = ChunkHeader::from_bytes(bytes)?;
