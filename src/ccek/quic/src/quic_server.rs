...
use crate::compat::{NetTuple, Protocol as RbProtocol, RbCursor, Signal as RbSignal};
...
mod tests {
    ...
    fn bind_installs_session_cache_in_context() {
        use crate::quic_session_cache::SessionCacheService;
        use crate::compat::CoroutineContext;
        ...
        let shared_cache = std::sync::Arc::new(crate::quic_session_cache::DefaultQuicSessionCache::default());
...
impl QuicServer {
    ...
    fn try_decrypt_1rtt_packet(
        packet_data: &[u8],
        dcid_len: usize,
        crypto_provider: &Arc<dyn crate::quic_crypto::QuicCryptoProvider>,
    ...
    pub async fn bind(addr: SocketAddr, ctx: crate::compat::CoroutineContext) -> Result<Self, QuicError> {
        let socket =
        ...
        let svc = SessionCacheService::new(shared_cache);
        let ctx = crate::compat::CoroutineContext::with_element(svc).merge(&ctx);
