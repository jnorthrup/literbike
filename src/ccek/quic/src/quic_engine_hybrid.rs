...
use crate::compat::cas_storage_compat::{
    ContentAddressedStore, ContentBlob, ContentHash, MerkleNode,
};
...
impl QuicContentLogger {
    ...
    fn flush_loop(
        ...
        loop {
            ...
            let hashes: Vec<ContentHash> = batch.iter().map(|e| e.content_hash.clone()).collect();
            ...
            // Store to DuckDB (batch insert)
            {
                ...
                for entry in &batch {
                    let blob =
                        ContentBlob::with_hash(entry.data.clone(), entry.content_hash.clone());
    ...
    fn create_entry(&self, pkt_num: u64, entry_type: LogEntryType, data: &[u8]) -> QuicLogEntry {
        let content_hash = ContentHash(Sha256::digest(data).to_vec());
        QuicLogEntry {
