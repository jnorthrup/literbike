...
pub struct QuicEngine {
    ...
    #[allow(dead_code)]
    ctx: crate::compat::CoroutineContext,
...
impl QuicEngine {
    ...
    pub fn new_with_crypto_provider(
        ...
        if let Some(tls_svc) = ctx.get_typed::<crate::tls_ccek::TlsCcekService>("TlsCcekService") {
            {
                ...
                if let Ok(server_conn) = rustls::quic::ServerConnection::new(tls_svc.config.clone(), rustls::quic::Version::V1, transport_params) {
                    ...
                    if let Ok(rustls_provider) = crate::tls_crypto::provider::RustlsCryptoProvider::new_server(
    ...
    pub fn set_stream_priority(&self, stream_id: u64, priority: crate::quic_protocol::StreamPriority) {
        if let Some(stream) = self.stream_states.lock().get_mut(&stream_id) {
    ...
    pub fn get_stream_stats(&self, stream_id: u64) -> Option<crate::quic_stream::StreamStats> {
        self.stream_states.lock().get(&stream_id).map(|stream| {
            crate::quic_stream::StreamStats {
    ...
    pub async fn send_stream_data_priority(
        ...
        stream_id: u64,
        data: Vec<u8>,
        priority: crate::quic_protocol::StreamPriority,
    ) -> Result<(), QuicError> {
    ...
    pub fn get_streams_by_priority(&self) -> Vec<QuicStreamState> {
        let mut streams: Vec<_> = self.stream_states.lock()
            .values()
            .filter(|s| s.state != crate::quic_protocol::StreamState::Closed)
            .cloned()
            .collect();
    ...
    pub fn create_stream_with_priority(&self, priority: crate::quic_protocol::StreamPriority) -> u64 {
...
mod tests {
    use super::*;
    use crate::quic_protocol::{serialize_packet, ConnectionId, TransportParameters};
    ...
    async fn process_packet_surfaces_inbound_header_hook_error() {
        ...
        let engine = QuicEngine::new_with_crypto_provider(
            ...
            crate::compat::CoroutineContext::new(),
        );
    ...
    async fn send_stream_data_rolls_back_state_when_udp_send_fails() {
        ...
        let engine = QuicEngine::new(Role::Client, sample_state(), socket, remote_addr, vec![], crate::compat::CoroutineContext::new());
    ...
    async fn process_stream_frame_fin_marks_remote_half_close_and_local_send_closes() {
        let socket = Arc::new(tokio::net::UdpSocket::bind("127.0.0.1:0").await.unwrap());
        let remote_addr: SocketAddr = "127.0.0.1:8888".parse().unwrap();
        let engine = QuicEngine::new(Role::Server, sample_state(), socket, remote_addr, vec![], crate::compat::CoroutineContext::new());
    ...
    async fn test_session_cache_put_after_handshake() {
        use crate::quic_session_cache::{DefaultQuicSessionCache, SessionCacheService};
        use std::sync::Arc;

        let shared_cache = Arc::new(DefaultQuicSessionCache::default());
        let svc = SessionCacheService::new(shared_cache.clone());

        let ctx = crate::compat::CoroutineContext::with_element(svc);
