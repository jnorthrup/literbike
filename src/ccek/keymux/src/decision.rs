...
use crate::env_facade_parity::{
    normalize_env_pairs, ModelmuxMvpLifecycle, NormalizedEnvProfile, QuotaInventorySlot,
};
...
mod tests {
    ...
    fn test_unified_mux_decision_env_first() {
        let env_pairs = vec![("KILO_API_KEY".to_string(), "sk-kilo".to_string())];
        let state =
            UnifiedMuxState::from_env_pairs(env_pairs).with_precedence(PrecedenceMode::EnvFirst);
    ...
    fn test_extract_provider_from_key() {
        assert_eq!(
            UnifiedMuxState::extract_provider_from_key("KILO_API_KEY"),
            "kilo"
        );
        assert_eq!(
            UnifiedMuxState::extract_provider_from_key("MOONSHOT_API_KEY"),
            "moonshot"
        );
    }
}
