...
impl QuicCcek {
    ...
    pub fn execute_reactor_continuation(
        &mut self,
        target_state: StateKey,
    ) -> Result<(), CcekError> {
        ...
        (transition.reactor_continuation)(ctx).map_err(|_| {
            CcekError::ReactorContinuationFailed(format!(
                "Transition {}->{}",
    ...
    pub fn navigate_key_graph(&self, from: StateKey, to: StateKey) -> Option<&ProtocolTransition> {
        self.key_graph
            .1
            .get(&from)
            .filter(|transition| transition.to_state == to)
    }

    pub fn should_emit_cover(&self, last: Instant, now: Instant) -> bool {
        if !self.policy.enable_cover {
            return false;
        }
