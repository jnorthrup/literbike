...
impl CursorManager {
    ...
    pub fn start_cleanup_task(&self) -> tokio::task::JoinHandle<()> {
        let cursor_manager = Arc::new(self.cursors.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let now = Utc::now();
                let mut cursors = cursor_manager.write().unwrap();
                
                let expired_keys: Vec<String> = cursors
                    .iter()
                    .filter(|(_, cursor)| now - cursor.timestamp > ttl)
                    .map(|(key, _)| key.clone())
                    .collect();
                ...
                if removed_count > 0 {
                    debug!("Background cleanup removed {} expired cursors", removed_count);
                }
            }
        })
    }
